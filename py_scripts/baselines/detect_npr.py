...
def load_data(input_file):
    if args.main_results:
        ...
        data_file = f"{input_file}_perturbation_{args.n_perturbations}.raw_data.json"
    else:
        # data_file = os.getcwd() + f"{input_file}_perturbation_{args.n_perturbations}_{args.scenario}.raw_data.json"
        data_file = f"{input_file}_perturbation_{args.n_perturbations}_{args.scenario}.raw_data.json"
...
def get_npr(args, scoring_model, scoring_tokenizer, text, perturbs):
    with torch.no_grad():
        ...
        for perturb in perturbs:
            tokenized = scoring_tokenizer(perturb, return_tensors="pt", return_token_type_ids=False).to(device)
            labels = tokenized.input_ids[:, 1:]
            logits = scoring_model(**tokenized).logits[:, :-1]
...
def experiment(args):
    ...
    #             'info': {'n_samples': n_samples, 'n_perturbations': args.n_perturbations,},
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_file', type=str, default="experiment_results/npr_detection_results/xsum_llama3_8b")
