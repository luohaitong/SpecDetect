...
def get_tau_multiscale_DE_stft(ori_data, epsilon, tau_prime):
    """
    example: ori_data = [1, 2, 13, 7, 9, 5, 4], embed_size=3, epsilon = 10,  tau = 3
             [1, 2, 13, 7, 9, 5, 4] ===> {'tau = 1': [1.0, 2.0, 13.0, 7.0, 9.0, 5.0, 4.0], 'tau = 2': [1.5, 7.5, 10.0, 8.0, 7.0, 4.5], 'tau = 3': [5.333333333333333, 7.333333333333333, 9.666666666666666, 7.0, 6.0]}
                                    ===> {'tau = 1': de([1.0, 2.0, 13.0, 7.0, 9.0, 5.0, 4.0]), 'tau = 2': de([1.5, 7.5, 10.0, 8.0, 7.0, 4.5]), 'tau = 3': de([5.333333333333333, 7.333333333333333, 9.666666666666666, 7.0, 6.0])}
    ...
    """
    # mde = torch.zeros(tau_prime)
    tau_list = [10,20,30,40]
    mde = []
    for temp_tau in tau_list:
        value = get_tau_scale_DE_stft(ori_data, epsilon, temp_tau)
        mde.append(value) 
    mde = torch.stack(mde, dim=0)
    std_mde = torch.std(mde, dim=0) 
    # print("mde.shape:",mde.shape)
    mean_mde = torch.mean(mde, dim=0)
    return std_mde
...
def get_tau_scale_DE_stft(log_likelihood, epsilon, tau):
    """
    example: ori_data = [1, 2, 13, 7, 9, 5, 4], embed_size=3, epsilon = 10,  tau = 2
             [1, 2, 13, 7, 9, 5, 4] ===> [1.5, 7.5, 10.0, 8.0, 7.0, 4.5] ==> de_value([1.5, 7.5, 10.0, 8.0, 7.0, 4.5])
    params:
    ...
    log_likelihood_tem = log_likelihood
    f, t, Sxx = _stft_power(log_likelihood_tem, nperseg=tau, window='hann', db_scale=False)

    sim = torch.nn.functional.cosine_similarity(torch.tensor(Sxx[:, :-1]), torch.tensor(Sxx[:, 1:]), dim=0)

    # 计算 sim 的波动情况（标准差）
    sim_std = torch.std(sim).unsqueeze(0)
    sim_mean = torch.mean(sim).unsqueeze(0)
    # sim = sim.unsqueeze(0).unsqueeze(-1)

    # batched_1 = torch.vmap(histcounts, in_dims=0, out_dims=1) 

    # epsilon = len(t)-1
    # # epsilon = 10
    # hist, statistical_probabilities_sequence = batched_1(sim, epsilon=epsilon)  
    # # calculate de
    # DE_value = DE(statistical_probabilities_sequence, epsilon)

    # print("DE_value:", DE_value.shape)
    # print("sim_std:", sim_std.shape)

    fluxes = np.sqrt(np.sum((Sxx[:, 1:] - Sxx[:, :-1]) ** 2, axis=0))
    mean_flux = np.mean(fluxes) if len(fluxes) > 0 else 0

    return torch.tensor(mean_flux).unsqueeze(0)
